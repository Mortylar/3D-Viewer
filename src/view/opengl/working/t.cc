...
static GLuint create_shader (int type, const char *src) {
  ...
  if (status == GL_FALSE) {
      ...
      g_print("log_len = %i\n", log_len);
      buffer = (char*)g_malloc (3000);//(log_len + 1);
...
static void init_shaders(const char *vertex_path, const char *fragment_path,
  ...
  int status;
  GBytes *source;
  char* sss = LoadFile(vertex_path);
  g_print("\nFile is = %s\n", sss);
  source = (GBytes*)sss;
  //source = g_resources_lookup_data (vertex_path, G_RESOURCE_LOOKUP_FLAGS_NONE, NULL);
  g_print("\nsource = %p\n", source); 
  vertex = create_shader(GL_VERTEX_SHADER, sss);
  //vertex = create_shader(GL_VERTEX_SHADER,(const char*)g_bytes_get_data(source, NULL));
  ...
  char* fff = LoadFile(fragment_path);
  //source = g_resources_lookup_data (fragment_path,G_RESOURCE_LOOKUP_FLAGS_NONE, NULL);
  //unsigned int b[10] = {1,1,1,0,1,1,1,0,0,0};
  //source = (GBytes*)b;
  fragment = create_shader(GL_FRAGMENT_SHADER, fff);
  //fragment = create_shader (GL_FRAGMENT_SHADER,(const char*)g_bytes_get_data(source, NULL));
  g_bytes_unref (source);
...
static void compute_mvp (float *res, float  phi,
...
static void realize (GtkWidget *widget) {
  ...
  if (gdk_gl_context_get_use_es (context))
    ...
    { 
      g_print("\ndont use es\n");
      vertex_path = "glarea-gl.vs.glsl";
      fragment_path = "glarea-gl.fs.glsl";
    }
...
static GtkWidget* create_glarea_window (GtkWidget *do_widget) {
...
static void activate(GtkApplication* app, gpointer user_data) {
  ...
  GtkWidget* area;
  area = do_glarea(window);
  gtk_window_set_child(GTK_WINDOW(window),create_glarea_window(window));
